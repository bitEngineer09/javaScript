...
let fruits4 = ["apple", "orange", "mango", "grapes", "banana"];
...
for (let item = 0; item < fruits4.length; item++) {
...
const myForEachArr = fruits.forEach((curElem, index, arr) => {
  ...
  //   console.log(arr); // this will prit whole array at every iteration
});
console.log(myForEachArr); // undefined
// forEach me fn ko variable me store karne me undefined milta hai, but in case of map, function result ko return kar deta hai
...
const myMapArr = fruits.map((curElem, index, arr) => {
  ...
  //   console.log(arr); // same as forEach
});

console.log(myMapArr); // will print the array
console.log(fruits);
...
numbers0.forEach((curElem) => {
  console.log(curElem * 2);
  //   if you have to only performs an action on each element
});

const doubleValue = numbers0.map((curElem) => {
  return curElem * 2;
